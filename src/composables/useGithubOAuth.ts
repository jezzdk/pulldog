...
export function useGithubOAuth() {
  ...
  function startRedirect(clientId: string): void {
    const oauthState = btoa(String.fromCharCode(...crypto.getRandomValues(new Uint8Array(16))))
    ...
    const params = new URLSearchParams({
      client_id: clientId,
      redirect_uri: import.meta.env.VITE_GITHUB_REDIRECT_URI || window.location.origin,
      scope: "repo read:org",
      state: oauthState,
    });
  ...
  async function handleCallback(): Promise<string | null> {
    ...
    if (!savedState || returnedState !== savedState) {
      state.value = { status: "error", message: "Authorization state mismatch — please try again" };
      return null;
    }
    ...
    if (!workerUrl) {
      state.value = { status: "error", message: "VITE_GITHUB_WORKER_URL is not configured" };
      return null;
    }
    ...
    try {
      ...
      state.value = {
        status: "error",
        message: data.error_description ?? data.error ?? "GitHub authorization failed",
      };
      return null;
    } catch {
      state.value = { status: "error", message: "Could not reach auth worker — try connecting with a token instead" };
      return null;
    }
  }
  ...
  async function revokeToken(token: string): Promise<void> {
    const workerUrl = import.meta.env.VITE_GITHUB_WORKER_URL;
    if (!workerUrl || localStorage.getItem(TOKEN_SOURCE_KEY) !== "oauth") return;
