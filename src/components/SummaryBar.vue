...
const mergeRate = computed((): number =>
  (props.created7d ?? 0) > 0
    ? Math.min(100, Math.round(((props.merged7d ?? 0) / props.created7d!) * 100))
    : 0,
)
</script>

<template>
  <div class="flex items-center gap-0 overflow-x-auto border-b border-border bg-card px-6">
    ...
    <div class="py-4 pr-8 shrink-0">
      ...
      <div class="font-mono text-[10px] text-muted-foreground mt-0.5">last 7 days</div>
    </div>
    ...
    <!-- Avg lead time -->
    <div class="py-4 pr-8 shrink-0">
      <div class="font-mono text-2xl font-bold tracking-tight leading-none mb-1" :class="leadClass">
      ...
      <div class="font-mono text-[10px] text-muted-foreground mt-0.5">open → merge, 7d</div>
    </div>
    ...
    >
      <span class="font-mono text-[10px] uppercase tracking-widest text-muted-foreground/60 whitespace-nowrap">
        Merge rate
      </span>
      <div class="w-24">
        <Progress :value="mergeRate" />
      </div>
      <span class="font-mono text-xs font-semibold text-purple-400 min-w-[3ch]">{{ mergeRate }}%</span>
    </div>
