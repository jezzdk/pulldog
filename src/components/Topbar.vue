...
import { RefreshCw, Settings, Volume2, VolumeX, Zap, Maximize2, Minimize2 } from 'lucide-vue-next'
...
<template>
  <header
    class="sticky top-0 z-40 flex h-14 items-center justify-between
           border-b border-border bg-background/90 backdrop-blur-md px-5"
  >
    <!-- Left: brand + stat pills -->
    <div class="flex items-center gap-4">
      <div class="flex items-center gap-2 font-mono text-base font-semibold text-foreground">
    ...
    <div class="flex items-center gap-2">
      <span v-if="lastUpdated" class="hidden lg:block font-mono text-xs text-muted-foreground/60">
        {{ lastUpdated }}
      </span>

      <Button variant="outline" size="sm" :class="loading ? 'opacity-60' : ''" @click="$emit('refresh')">
      ...
      <Button variant="ghost" size="icon" title="Settings" @click="$emit('openSettings')">
