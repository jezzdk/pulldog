...
const hoursOpen = computed(() => Math.floor((Date.now() - props.createdAt.getTime()) / 3_600_000))
...
<template>
  <div v-if="status !== 'ok'">
    <Badge
      ...
      :class="status === 'breach' ? 'animate-breach-pulse border-destructive/30' : 'border-yellow-500/30'"
    >
